...
public class SetupScene extends Scene{
    public SetupScene(Group rootGroup, Stage calcStage) {
        super(rootGroup);
        ...
        rootGroup.getChildren().add(layoutGrid);

        //sets stage specifications and shows the stage
        calcStage.setTitle("EuroCalc");
        calcStage.setResizable(true);
        calcStage.setMaxHeight(200);
        calcStage.setMaxWidth(300);
        calcStage.setScene(this);
        calcStage.show();
    }
    ...
    private static double currencyCoef(String currency){
        ...
        try(
                Scanner fileScanner = new Scanner(new File("src/main/resources/currencies.txt"))
        ){
            String line;
